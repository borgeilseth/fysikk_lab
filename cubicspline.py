...
f = np.array((2 / 7) * 0.031 * 9.81 * np.sin(b))
fExp = np.array((2 / 7) * 0.031 * 9.81 * np.sin(bExp))

fn = np.abs(f / n)
fnExp = np.abs(fExp[1:-1] / nExp)

friksjonsforhold = plt.figure("y(x)", figsize=(12, 6))
plt.plot(x, fn)
plt.plot(xExp[2:-2], fnExp)
# plt.plot(xExp[2:-2], nExp)
plt.xlabel("$x$ (m)", fontsize=20)
...
tidFart = plt.figure("y(x)", figsize=(12, 6))
plt.plot(t, v)
plt.plot(tExp, vExp)
# plt.plot(xExp[2:-2], nExp)
plt.xlabel("$t$ (s)", fontsize=20)
...
friksjonsforhold.savefig("Friksjonsforhold.png", bbox_inches="tight")
tidFart.savefig("Fartsutvikling.png", bbox_inches="tight")
